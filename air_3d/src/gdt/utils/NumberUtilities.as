package gdt.utils
{
   ...
   */
		...
		/**
		 * Получение числа с разделителями
		 * @param number - исходное число
		 * @param separator - строка, которая будет использована в качестве разделителя
		 * @return Строку, представляющую исходное число, разделенное заданным разделителем
		 */
		public static function getSeparatedNumber(number:Number, separator:String = '.'):String
		{
			var str:String = '';
			
			var arrNumber:Array = number.toString().split('');
			...
			for (var i:int=0;i<arrNumber.length;i++)
			{
				...
				count ++;
				if (count == 3 && arrNumber.length != i + 1)
				{
					count = 0;
					arr.push(separator);
				}
