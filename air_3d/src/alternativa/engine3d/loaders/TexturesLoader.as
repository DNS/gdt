...
package alternativa.engine3d.loaders {
	...
	public class TexturesLoader extends EventDispatcher {
		...
		public var fromAsset:Boolean = false;
		...
		private function loadNext(e:* = null):void {
//			trace("[NEXT]", e);
			...
			if (e != null && !(e is ErrorEvent)) {
				if (isATF) {
					...
					bitmapData = fromAsset ? e.bitmapData : e.target.content.bitmapData;
		...
		private function load(url:String):void {
			/*if (fromAsset) {
				trace("fromAsset");
				var bitmap:Bitmap = new Bitmap(Application.assets.getAsset(url).getContent().bitmapData);
				loadNext(bitmap);
				return;
				}*/
